"""Independent wall solution implementation.

This module implements the Independent Wall soundproofing solution, which provides
superior sound isolation through a completely separate wall structure. The solution
uses the following key components and patterns:

1. Inheritance Structure:
   - Inherits from BaseWall for common wall functionality
   - Provides specialized calculations for independent wall construction
...
3. Key Features:
   ...
   - Automated stud spacing and quantity calculations
...
class IndependentWallStandard(BaseWall):
    """Standard Independent Wall solution.
    
    This class implements the standard independent wall solution with the following features:
    - Double layer plasterboard construction
    - Standard stud spacing optimization
    - Material quantity calculations with waste factors
    - Cached characteristics for performance
    
    The calculation process:
    1. Base calculations from BaseWall (dimensions, basic materials)
    2. Stud spacing and quantity optimization
    3. Plasterboard quantity calculation with waste factor
    4. Additional component calculations
    """
    
    CODE_NAME = "Independent Wall (Standard)"
    STUD_SPACING = 0.4  # 400mm spacing for wall studs
    ...
    def calculate(self, dimensions: Optional[Dict] = None, materials: Optional[List] = None) -> Dict[str, Any]:
        """Calculate solution properties with wall-specific additions"""
        try:
            ...
            # Calculate area and stud quantities
            area = length * height
            studs_needed = math.ceil(length / self.STUD_SPACING)
            extra_studs = math.ceil(studs_needed * 0.1)  # 10% extra for cuts
            ...
            logger.error(f"Error calculating IndependentWall: {e}")
            return None
...
class IndependentWallSP15(BaseSP15Solution, IndependentWallStandard):
    """SP15 variant of Independent Wall solution.
    
    This variant enhances the standard independent wall with:
    - SP15 soundboard integration
    - Enhanced isolation studs
    ...
    def calculate(self, dimensions: Optional[Dict] = None, materials: Optional[List] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result['additional_info'].update({
                ...
                'plasterboard_layers': self.plasterboard_layers,
                'enhanced_studs': True    # SP15 uses enhanced isolation studs
            })
            
            return result
            
        except Exception as e:
            logger.error(f"Error calculating SP15 IndependentWall: {e}")
            return None
        ...
        try:
            ...
            if characteristics:
                ...
                characteristics.update({
                    ...
                    'variant': 'SP15',
                    'description': 'Enhanced Independent Wall solution with improved sound isolation'
                })
...
def load_independentwall_solutions():
    """Load Independent Wall solutions (Standard and SP15) from MongoDB and cache them."""
    from solutions.database import diagnose_missing_document
    cache_manager = get_cache_manager()
    ids = SOLUTION_VARIANT_MONGO_IDS['walls']
    standard_id = ids['IndependentWallStandard']
    sp15_id = ids['IndependentWallSP15']
    standard_doc = get_solution_by_id('wallsolutions', standard_id)
    if standard_doc:
        print(f"[DEBUG] IndependentWallStandard ({standard_id}) keys: {list(standard_doc.keys())}")
    else:
        print(f"[WARN] IndependentWallStandard ({standard_id}) not found.")
    sp15_doc = get_solution_by_id('wallsolutions', sp15_id)
    if sp15_doc:
        print(f"[DEBUG] IndependentWallSP15 ({sp15_id}) keys: {list(sp15_doc.keys())}")
    else:
        print(f"[WARN] IndependentWallSP15 ({sp15_id}) not found.")
    standard = IndependentWallStandard() if standard_doc else None
    sp15 = IndependentWallSP15() if sp15_doc else None
    if standard:
        standard._solution_data = standard_doc
        cache_manager.set('independentwall_standard', standard, 3600)
    if sp15:
        sp15._solution_data = sp15_doc
        cache_manager.set('independentwall_sp15', sp15, 3600)
